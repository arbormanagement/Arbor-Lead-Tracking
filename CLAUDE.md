...
- Ad-level drill-down (`ad_spend_ads`, keyed `(platform, external_ad_id, date)`) syncs alongside campaign spend — Google `ad_group_ad` + FB `level=ad` insights with creative previews. Per-ad ROAS joins `leads.external_ad_id` (FB lead-gen carries it natively; web + DNI calls need the `utm_content` ad-id stamp — setup in `docs/ad-level-tracking.md`). ROI rollup/attribution stays campaign-level; LSA/PMax have no per-ad rows.
