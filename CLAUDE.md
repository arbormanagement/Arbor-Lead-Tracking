...
- **Vocabulary (settled 2026-09-05).** An **inquiry** is one episode of a person reaching out —
  ...
  rename would leave the snapshot disagreeing with the schema. **The prompt can be driven from
  a script** — migration 0056 (a column rename) was generated by running `drizzle-kit generate`
  under a pseudo-terminal and answering it, see DEPLOY.md — so the table rename is a
  scheduling question, not a blocked one. Do it when the aliases retire on 2026-10-05.
...
- **A crawl cannot see an absence.** It only ever reads what HCP still returns, so a
  ...
  **Since 2026-09-05 the sync TOMBSTONES them** (`missing_from_hcp_at`, all four tables,
  migration 0056): when a provably-full lap wraps, `tombstoneMissing` stamps every row the
  lap did not see, and `markCrawlSeen` clears the stamp the moment a crawl sees the row
  again. Nothing is deleted — a merged customer's estimates, jobs and contacts still
  reference the row — but every reader that means "customers HCP has" now filters on it:
  `arbor_list_customers` (live by default, `missingFromHcp: true` for the audit view, and the
  mailable set can never include one), `link-hcp` (so the surviving record of a merge wins,
  not the older tombstone), the `do_not_service` unknown count, and `hcpSync.<name>.ours`, so
  drift reads 0 while `tombstoned` carries the fact. Two guards keep it honest: a lap may not
  tombstone more than max(100, 2%) of a table on its own (the 2026-08-26 false alarm would
  have condemned 14,000 estimates; a refusal is logged, reported as a NEGATIVE `tombstoned`
  in the run stats, and left under `missingFromHcp` for a human), and ANY tombstoned
  estimate warns, because HCP never removes an estimate from its API, so one can only mean
  the crawl skipped a page. The first real lap after the deploy stamps the 62 customers HCP
  had merged away and both standing warnings clear. `npm run verify:hcp` covers all of it.
...
- **A promoted channel does NOT reclassify the leads already in `other`.** `classifySource`
  ...
  **`POST /api/admin/reclassify-sources?apply=true`** for production, which is the only way to
  ...
  - **⚠️ It could not rescue a session-backed lead on its SOURCE until 2026-09-05.**
    `web_sessions.source_key` (named `source` until migration 0056 the same day) holds the
    CLASSIFIED key (the output of `classifySource` at ingest), and
    ...
    the classified column. The column was renamed `source_key` (migration 0056) so the next
    reader cannot mistake it for the raw tag.
