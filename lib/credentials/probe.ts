...
/**
 * Lightweight connectivity probe per platform, using the resolved credentials.
 *
 * Each probe is the cheapest authenticated call that proves a credential actually
 * WORKS — which is the whole point, and the reason Settings → Integrations is
 * status plus a Test button rather than status alone. `configured` only says a
 * value is present; only a real call distinguishes a working key from a typo.
 *
 * Shared by /api/settings/credentials/test and the MCP `arbor_test_credentials`
 * tool. Never returns or logs the credential itself.
 */
export async function probeCredential(platform: string): Promise<{ ok: boolean; error?: string; detail?: string }> {
