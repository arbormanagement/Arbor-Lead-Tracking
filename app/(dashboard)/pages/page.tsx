 ...
 *
 * **No spend column, and that is the point.** Money attaches to a campaign, not to
 * a landing page; splitting it across pages would be arbitrary, and no serious
 * attribution system does it. The question here is a RATE — of the people who
 * landed on this page, how many got in touch — and rates need the visitors who did
 * NOT convert in the denominator, which is why this reads `web_sessions` rather
 * than working backwards from leads.
 *
 * That denominator is also what makes this answerable at Arbor's volume. A
 * page-level revenue comparison needs on the order of a hundred conversions per
 * page and would be noise for a year; a session→contact rate has thousands of
 * sessions behind it and separates pages within weeks.
 *
 * The column that earns this page its keep is **Close** — of the contacts a page
 ...
 * contacts that actually close, because that outcome lives in HousecallPro. A page
 * can produce fewer contacts and better ones, and that is the finding worth acting
 * on.
 */
export default async function PagesPage({ searchParams }: { searchParams: Promise<{ days?: string }> }) {
      ...
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">
            Which landing pages turn visits into work · {timeframeLabel(days)}
          </p>
        </div>
        <div className="controls">
          ...
          {TIMEFRAMES.map((t) => (
            <Link
              key={t.days}
              href={t.days === 30 ? "/pages" : `/pages?days=${t.days}`}
              className="pill"
              style={
                days === t.days
      ...
      {rows.length === 0 ? (
        ...
        <>
          ...
          <p className="page-sub" style={{ fontSize: 12.5 }}>
            ...
            <Link href="/roi" className="link">Campaigns</Link> for return on spend. Conversion is withheld under{" "}
